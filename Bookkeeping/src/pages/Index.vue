<template>
  <div class="index">
    <index-top-bgc>
      <index-top-btn>
        <a slot="bintu" :style="recColor">
          <van-icon name="clock-o" @click="changeRec"></van-icon>
        </a>
        <a slot="xianz" :style="redColor">
          <van-icon name="chart-trending-o" @click="changeRed"></van-icon>
        </a>
      </index-top-btn>
    </index-top-bgc>
    <div class="index-rad-bgc-div" v-if="redChange">
      <index-rad-bgc>
        ...
        <rad-bgc-doub v-if="changeDoub"></rad-bgc-doub>
        <div class="all-count">
    ...
    <div class="index-rec-bgc-div" v-if="recChange">
      <index-rec-bgc>
        <div class="index-rec-list">
          <index-rec-bgc-list></index-rec-bgc-list>
        </div>
      </index-rec-bgc>
    </div>
        ...
        .then(res => {
          ...
          for (var i = 0; i < res.data.data.list.length; i++) {
            if (res.data.data.list[i].type == "2") {
              ...
              var lessmoney = Number(
                res.data.data.list[i].total_money.split(".")[0]
              );
              resData[j] = {
                type: res.data.data.list[i].company_name,
                value: lessmoney
              };
              j++;
            }
