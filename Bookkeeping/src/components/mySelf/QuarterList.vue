<template>
  <div class="MonthList">
    <div class="MonthList-body" id="div">
...
export default {
  ...
  methods: {
    handleScroll(e) {
      ...
      if (
        scrollTop + clientHeight - docHeight > -10 &&
        scrollTop + clientHeight - docHeight < 10
      ) {
        this.y++;
        //   console.log(this.y)
    ...
    getData: function() {
      ...
      let monthstr = Number(endstr.split("-")[1]);
      let theYear = moment(data).format("YYYY");
      if (monthstr >= 1 && monthstr <= 3) {
        this.begindata = theYear + "-01-01";
        this.enddata = theYear + "-03-31";
      } else if (monthstr >= 4 && monthstr <= 6) {
        this.begindata = theYear + "-04-01";
        this.enddata = theYear + "-06-30";
      } else if (monthstr >= 7 && monthstr <= 9) {
        this.begindata = theYear + "-07-01";
        this.enddata = theYear + "-09-30";
      } else {
        this.begindata = theYear + "-10-01";
        ...
        .then(res => {
          ...
          for (var i = 0; i < this.paidName.length; i++) {
            this.paidName[i].lessMoney =
              Number(res.data.data.list[i].total_money.split(".")[0]) -
              Number(res.data.data.list[i].paid_money.split(".")[0]);
          }
          //   console.log(this.paidName)
          Toast.loading({
            mask: false,
