  ...
  <div class="Balance-index">
    ...
    <van-popup v-model="show" position="top" :style="{ height: '30%' }">
      ...
      <div class="createNewBookBtn">
        <van-button type="primary" color="rgb(236,89,89)" size="normal" @click="CreateAccountBook">创建新的账簿</van-button>
      </div>
    </van-popup>
    <div class="popup-div">
      <van-popup v-model="letShow">
        ...
        <div class="popup-div-btn">
          <van-button type="primary" color="rgb(236,89,89)" size="small" @click="pushUpdata">提交修改</van-button>
          <van-button type="primary" color="rgb(236,89,89)" size="small" @click="passUpdata">放弃修改</van-button>
        </div>
      </van-popup>
    </div>
    <div class="popup-div2">
      <van-popup v-model="CreateShow">
        ...
        <div class="popup-div-btn">
          <van-button type="primary" color="rgb(236,89,89)" size="small" @click="pushCreate">开始创建</van-button>
          <van-button type="primary" color="rgb(236,89,89)" size="small" @click="passCreate">放弃创建</van-button>
        </div>
      </van-popup>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    changeAccountBook: function(book) {
      ...
      Dialog.confirm({
        ...
        .then(() => {
          axios
            .post(
              apiConfig.SettingCurrentAccount+"?token=" +
                localStorage.token,
              qs.stringify({
                book_id: book.id
              })
    ...
    delAccount: function(delbook) {
      Dialog.confirm({
        ...
        .then(() => {
          axios
            .post(
              apiConfig.DeleteAccounBooks+"?token=" +
                localStorage.token,
              qs.stringify({
                book_id: delbook.id
              })
    ...
    pushUpdata: function() {
      axios
        .post(
          apiConfig.AlterAccountBook+"?token=" +
            localStorage.token,
          qs.stringify({
    ...
    pushCreate: function() {
      ...
      axios
        .post(
          apiConfig.CreateAccountBook+"?token=" +
            localStorage.token,
          qs.stringify({
